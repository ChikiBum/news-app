...
const AD_SIZES = [[300, 250], [250, 250]];
...
function loadAdtelligentBidAdapter() {
  ...
  return appendScriptWithSrc("http://localhost:4444/bundle?modules=adtelligentBidAdapter");
}
...
function loadPbjsScript() {
  ...
  const debugLogCode = localStorage.getItem('adsDebug') === 'true' 
    ? `
...
function loadDisplaySlot() {
  ...
  adsContainers.forEach(container => {
    ...
    div.id = adId;
    div.style.cssText = "width: 300px; height: 250px; border: 1px solid #ddd; background-color: #f5f5f5; margin: 0 auto;";
  ...
  debugLog(`📺 Обработано ${processedCount} новых контейнеров из ${adsContainers.length} найденных`);
  return Promise.resolve();
}

function runAllAdsSequence() {
  debugLog('🚀 Запускаем последовательность загрузки рекламы через 2 секунды...');
  setTimeout(() => {
    debugLog('⏰ 2 секунды прошло, начинаем цепочку загрузки рекламы');
    loadAdtelligentBidAdapter()
      .catch(err => {
        debugWarn('⚠️ AdtelligentBidAdapter не загрузился, продолжаем без него:', err.message);
        return Promise.resolve(); 
      })
      .then(() => {
      ...
      .catch(err => {
        ...
        if (err.message && err.message.includes('googletagservices.com')) {
          ...
          debugError("💡 Возможные причины: блокировка рекламы, проблемы с сетью");
        }
      });
  }, 2000);
}
